...
from config.constants import MODEL_COLLECTION, CLEANED_FEATURE_COLLECTION
...
def _load_latest_regression_model(target: str = "target_aqi_t_plus_24h"):
    ...
    # GridFS load
    gridfs_id = model_doc.get("gridfs_file_id")
    if not gridfs_id:
        raise ValueError(f"No GridFS file ID found in registry for {target}")
    ...
    try:
        ...
        raise ValueError(f"Failed to load model from GridFS: {e}")
...
def compute_global_shap(top_n=15, sample_size=None, target="target_aqi_t_plus_24h"):
...
def compute_local_shap_from_dashboard(latest_features: dict, target="target_aqi_t_plus_24h"):
...
def compute_local_shap(target="target_aqi_t_plus_24h"):
